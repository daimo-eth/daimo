 ...
 *
 * V2: Allow claims to be made by an address other than msg.sender
 */
contract DaimoEphemeralNotesV2 {
    ...
    // creator can reclaim the note without a signature
    function claimNoteSelf(address _ephemeralOwner) external {
        ...
        require(msg.sender == note.from, "EphemeralNotes: not note sender");
    ...
    // recipient (with a valid signature) can redeem the note
    // via someone else's call to this function
    function claimNoteRecipient(
        ...
        emit NoteRedeemed(note, recipient);
        delete notes[_ephemeralOwner];
        token.safeTransfer(recipient, note.amount);
    }
}
