...
contract SwapperTest is Test {
    ...
    function testSwapToBridgeableCoin() public {
        ...
        (quotedAmountOut, swapPath) = swapper.quote(amountIn, tokenIn, usdc);
        assertEq(quotedAmountOut / 10 ** 4, expectedAmountOut / 10 ** 4); // in cents

        // Check that the last token in the swap path is USDC.
        IERC20 tokenOutInSwapPath = swapper.getFinalOutputToken(swapPath);
        assertEq(address(tokenOutInSwapPath), address(usdc));
