...
contract P256SHA256Test is Test {
    ...
    function testTrivialVerify() public {
        ...
        bytes memory prefixedData = bytes.concat(
            "\x19Ethereum Signed Message:\n32",
            data
        );
        bytes
            memory signature = hex"93dfb3f8d2af9e0f55067293dfcae5ae1bd480e6f8afd2617d160978511bb216a2ed2f7f77d9a7e8ff44bc47a4d8f6aee50340cc01dbb0843c39562dfa0d9ed5";
        bool ret = verifier.verify(key, prefixedData, signature);
        assertTrue(ret);

        data = hex"deadbeef";
        prefixedData = bytes.concat("\x19Ethereum Signed Message:\n32", data);
        ret = verifier.verify(key, prefixedData, signature);
        assertFalse(ret);
    }
}
